...
import {
  Form, Modal, Input, Button, Tag, message,
} from 'antd';
...
class WhiteListModal extends Component {
  ...
  async fetchDetail(id) {
    ...
    this.setState({ blackList, whiteList });
  }
  ...
  handleAdd = (type) => () => {
    ...
    if (type === 'whiteNo') {
      if (whiteList.every((item) => item !== value)) {
        this.setState({ whiteList: [...whiteList, value] });
      } else {
        message.warning('不能重复添加');
        return;
      }
    } else if (blackList.every((item) => item !== value)) {
      this.setState({ blackList: [...blackList, value] });
    } else {
      message.warning('不能重复添加');
      return;
    }
  ...
  render() {
    const { visible, form, disabled } = this.props;
    const { whiteList, blackList } = this.state;
      ...
      >
        <Form style={{ maxHeight: '300px', overflowY: 'auto' }}>
          <FormItem label="用户编号" {...formItemLayout}>
            {
              getFieldDecorator('whiteNo', {})(<Input disabled={disabled} style={{ width: '300px' }} />)
            }
          ...
          <FormItem label="白名单列表" {...formItemLayout}>
            {
              whiteList.map((item) => (
                <Tag key={item} closable={!disabled} onClose={this.handleClose(item, 'white')}>
                  {item}
                </Tag>
              ))
            }
          ...
          <FormItem label="用户编号" {...formItemLayout}>
            {getFieldDecorator('blackNo', {})(<Input disabled={disabled} style={{ width: '300px' }} />)}
            <Button className="ml10" disabled={disabled} onClick={this.handleAdd('blackNo')}>添加</Button>
          </FormItem>
          <FormItem label="黑名单列表" {...formItemLayout}>
            {
              blackList.map((item) => (
                <Tag key={item} closable={!disabled} onClose={this.handleClose(item, 'black')}>
                  {item}
                </Tag>
              ))
            }

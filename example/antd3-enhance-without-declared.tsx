...
const f15 = { fontSize: '15px' };
const ml175 = { marginLeft: '175px' };
const w250 = { width: '250px' };
const w375 = { width: '375px' };
const w295 = { width: '295px' };
const w530 = { width: '530px' };
const mb100 = { marginBottom: '100px' };
const mb200 = { marginBottom: '200px' };
const mb250 = { marginBottom: '250px' };
const mb350 = { marginBottom: '350px' };
const insertList = [
  { value: '{amount}', label: 'amount' },
  { value: '{startAddress}', label: '订单上车地点' },
  { value: '{endAddress}', label: '订单目的地' },
];
...
class ActivityStepTwo extends Component {
  ...
  goNext = () => {
    this.props.form.validateFieldsAndScroll({ force: true }, async (err) => {
      ...
      if (!err) {
        ...
        const { activityRuleDTOs = [], startTime, endTime } = data;
        const crowdArr = activityRuleDTOs.find((item) => item.ruleCode === 'PARTICIPATE_CROWD_TAG')
          ?.ruleContent?.tags?.map((item) => item.tag);
        const validator = await crowdValidator(crowdArr, 0, { startTime, endTime });
  ...
  render() {
    const {
      backgroundColor, insideTitle, insideHeadPic, insideDesc, insideButtonPic,
      outsideTitle, outsideDesc, outsideHeadPic, outsideButtonPic, outsideButtonWithoutPhonePic,
      outsidePrizeOldTitle, outsidePrizeOldDesc, outsidePrizeOldHeadPic, outsidePrizeOldButtonPic,
      outsidePrizeOldIsShareButtonPic, outsidePrizeOldIsShareButton, outsidePrizeEndTitle, outsidePrizeEndDesc, outsidePrizeEndHeadPic, outsidePrizeEndButtonPic,
      outsidePrizeNewTitle, outsidePrizeNewDesc, outsidePrizeNewHeadPic, outsidePrizeNewButtonPic,
      outsideNewAwardCopyWriter, endTitle, endDesc, endHeadPic, endButtonPic,
      insideIconPic, insideShareTitle, insideShareDesc, outsideShareTitle, outsideShareDesc,
      carIconPic, appWaitDocUrl, appWaitUrlType, appWaitIconPic,
      appWaitIconPicType,
      appWaitInteractiveType, appWaitContentOfMessage, appWaitIconPicOfMessage, appWaitUrlTypeOfMessage,
      appWaitDocUrlOfMessage,
      miniProgramWaitIconPicType, miniProgramWaitIconPic, miniProgramWaitUrlType, miniProgramWaitDocUrl,
      miniProgramWaitInteractiveType,
      evaluationDialogType, evaluationDialogJumpType, evaluationDialogJumpLink, evaluationDialogPic,
      orderShareType, waitEvaluationDialogType, waitEvaluationDialogDesc, uiType,
      travelingRedBagSignal, travelingRedBagIconPic, travelingRedBagUrlType,
      activityProgressNotification, pActivityName, pActivityProcess, pNextTarget,
      noticeActivityReward, rActivityName, rActivityProcess, rReminder, appletIconPic,
      // 评价成功banenr 相关字段
      evaluationBannerType, evaluationBannerPic, evaluationBannerJumpType, evaluationBannerJumpLink,
    } = this.props.interactiveDetail;
    const {
      supportShare, oldAcceptRule, externalOldRandomType, participateChannelList = [],
    } = this.props.redBagCarDetail;
    const { loading } = this.state;
    const { getFieldDecorator } = this.props.form;

    const checkboxPanelConfig = getPositionPanelConfig(supportShare, evaluationDialogJumpType,
      travelingRedBagUrlType, participateChannelList);
    ...
    return (
      <div>
        <Form className="rel" style={w530}>
          <FormItem {...formItemLayout} label="车辆图标配置">
            {
              getFieldDecorator('carIconPic', {
                ...
                rules: [{ validator: this.checkUploadPic('carIconPic') }],
                initialValue: carIconPic ? this.getDefaultFileListByKey('carIconPic') : '',
              })(<Upload
                {...uploadConfig}
                className="upload-list-inline"
                listType="picture"
                onChange={this.onUploadChange('carIconPic')}
                onRemove={this.onRemove('carIconPic')}
              >
                <Button>
            ...
            participateChannelList.includes('1') && (
              <>
                <FormItem {...formItemLayout} label="app等待接驾页面">
                  <div style={{ fontSize: '18px', fontWeight: '500' }}> 曹操出行App </div>
                  {
                ...
                {/* app 弹窗配置 */}
                {
                  appWaitIconPicType === 2 && appWaitInteractiveType === 1
                  && (
                    <FormItem className="mb0" {...tailFormItemLayout}>
                      {
                        getFieldDecorator('appWaitIconPic', {
                          valuePropName: 'fileList',
                          getValueFromEvent: this.normFile,
                          rules: [{ validator: this.checkUploadPic('appWaitIconPic') }],
                          initialValue: appWaitIconPic ? this.getDefaultFileListByKey('appWaitIconPic') : '',
                        })(<Upload
                          {...uploadConfig}
                          className="upload-list-inline"
                          listType="picture"
                          onChange={this.onUploadChange('appWaitIconPic')}
                          onRemove={this.onRemove('appWaitIconPic')}
                          beforeUpload={this.beforeUpload(810, 1080, 1024, false)}
                        >
                          <Button>
                ...
                {
                  appWaitIconPicType === 2 && appWaitUrlType === 2 && appWaitInteractiveType === 1
                  && (
                    <FormItem {...tailFormItemLayout}>
                      {
                        getFieldDecorator('appWaitDocUrl', {
                          rules: [{ required: true, message: '请填写跳转的url' }],
                  ...
                  && (
                    <>
                      <FormItem className="mb0" {...formItemLayout} label="消息条文案">
                        <p style={{ color: 'red' }}>消息条仅乘客端5.1.8及以上版本可展示，老版本默认不展示</p>
                        {
                      ...
                      <FormItem className="mb0" {...formItemLayout} label="logo图片">
                        ...
                        getFieldDecorator('appWaitIconPicOfMessage', {
                          valuePropName: 'fileList',
                          getValueFromEvent: this.normFile,
                          rules: [{ validator: this.checkUploadPic('appWaitIconPicOfMessage') }],
                          initialValue: appWaitIconPicOfMessage ? this.getDefaultFileListByKey('appWaitIconPicOfMessage') : '',
                        })(<Upload
                          {...uploadConfig}
                          className="upload-list-inline"
                          listType="picture"
                          onChange={this.onUploadChange('appWaitIconPicOfMessage')}
                          onRemove={this.onRemove('appWaitIconPicOfMessage')}
                          beforeUpload={this.beforeUpload(90, 90, 1024, false)}
                        >
                          <Button>
                            <Icon id="waitPageAnchor" className="c-anchor" type="upload" style={f15} /> 上传文件
                          </Button>
                ...
                {
                  appWaitIconPicType === 2 && appWaitUrlTypeOfMessage === 2 && appWaitInteractiveType === 2
                  && (
                    <FormItem {...tailFormItemLayout}>
                      {
            ...
            participateChannelList.includes('12') && (
              <>
                <FormItem {...formItemLayout} label="小程序等待接驾页面">
                  <div style={{ fontSize: '18px', fontWeight: '500' }}> 微信小程序 </div>
                  {
                ...
                {
                  miniProgramWaitIconPicType === 2 && miniProgramWaitInteractiveType === 1
                  && (
                    <FormItem className="mb0" {...tailFormItemLayout}>
                      {
                        getFieldDecorator('miniProgramWaitIconPic', {
                          valuePropName: 'fileList',
                          getValueFromEvent: this.normFile,
                          rules: [{ validator: this.checkUploadPic('miniProgramWaitIconPic') }],
                          initialValue: miniProgramWaitIconPic ? this.getDefaultFileListByKey('miniProgramWaitIconPic') : '',
                        })(<Upload
                          {...uploadConfig}
                          className="upload-list-inline"
                          listType="picture"
                          onChange={this.onUploadChange('miniProgramWaitIconPic')}
                          onRemove={this.onRemove('miniProgramWaitIconPic')}
                          beforeUpload={this.beforeUpload(810, 1080, 1024, false)}
                        >
                          <Button>
                            <Icon id="waitPageAnchor" className="c-anchor" type="upload" style={f15} /> 上传文件
              ...
              <FormItem {...tailFormItemLayout}>
                {
                  getFieldDecorator('travelingRedBagIconPic', {
                    valuePropName: 'fileList',
                    getValueFromEvent: this.normFile,
                    rules: [{ validator: this.checkUploadPic('travelingRedBagIconPic') }],
                    initialValue: travelingRedBagIconPic ? this.getDefaultFileListByKey('travelingRedBagIconPic') : '',
                  })(<Upload
                    {...uploadConfig}
                    className="upload-list-inline"
                    listType="picture"
                    onChange={this.onUploadChange('travelingRedBagIconPic')}
                    onRemove={this.onRemove('travelingRedBagIconPic')}
                    beforeUpload={this.beforeUpload(1194, 432, 200)}
                  >
                    <Button>
                      <Icon id="waitPageAnchor" className="c-anchor" type="upload" style={f15} /> 上传文件
                    </Button>
              ...
              <FormItem className="mb0" {...tailFormItemLayout}>
                {
                  getFieldDecorator('evaluationDialogPic', {
                    valuePropName: 'fileList',
                    getValueFromEvent: this.normFile,
                    rules: [{ validator: this.checkUploadPic('evaluationDialogPic') }],
                    initialValue: evaluationDialogPic ? this.getDefaultFileListByKey('evaluationDialogPic') : '',
                  })(<Upload
                    {...uploadConfig}
                    className="upload-list-inline"
                    listType="picture"
                    onChange={this.onUploadChange('evaluationDialogPic')}
                    onRemove={this.onRemove('evaluationDialogPic')}
                    beforeUpload={this.beforeUpload(810, 1080, 1024, false)}
                  >
                    <Button>
          ...
          {
            evaluationDialogType === 1 && evaluationDialogJumpType === 2
            && (
              <FormItem {...tailFormItemLayout}>
                {
          ...
          {
            participateChannelList?.includes('1') && (
            <FormItem {...formItemLayout} label="评价成功banner">
              {
                ...
                <FormItem className="mb0" {...tailFormItemLayout}>
                  {
                    getFieldDecorator('evaluationBannerPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('evaluationBannerPic') }],
                      initialValue: evaluationBannerPic ? this.getDefaultFileListByKey('evaluationBannerPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      className="upload-list-inline"
                      listType="picture"
                      onChange={this.onUploadChange('evaluationBannerPic')}
                      onRemove={this.onRemove('evaluationBannerPic')}
                      beforeUpload={this.beforeUpload(1029, 240, 1024, false)}
                    >
                      <Button>
          ...
          <FormItem {...formItemLayout} label="订单分享有礼" className="mb0">
            {
              getFieldDecorator('orderShareType', {
                rules: [{ required: true, message: '请选择是否开启订单分享有礼' }],
          ...
          {
            waitEvaluationDialogType === 1
            && (
              <FormItem {...tailFormItemLayout} className="mb0">
                {
                  getFieldDecorator('waitEvaluationDialogDesc', {
                    rules: [
                      {
                        required: true, message: '请填写待评价弹窗文案',
                      },
                      {
          ...
          {
            (evaluationDialogJumpType === 4 || travelingRedBagUrlType === 4)
            && (
              <div>
                <h4 id="insidePrizeAnchor">站内领奖页面</h4>
                <div className="rel"
                  style={supportShare === 2 ? mb200 : mb350}
                >
                  ...
                  <FormItem {...formItemLayout} label="头图">
                    {
                      getFieldDecorator('insideHeadPic', {
                        valuePropName: 'fileList',
                        getValueFromEvent: this.normFile,
                        rules: [{ validator: this.checkUploadPic('insideHeadPic') }],
                        initialValue: insideHeadPic ? this.getDefaultFileListByKey('insideHeadPic') : '',
                      })(<Upload
                        {...uploadConfig}
                        onChange={this.onUploadChange('insideHeadPic')}
                        beforeUpload={this.beforeUpload(1125, 480, 400)}
                        onRemove={this.onRemove('insideHeadPic')}
                      >
                        <Button>
                      ...
                      <FormItem {...formItemLayout} label="分享button">
                        {
                          getFieldDecorator('insideButtonPic', {
                            valuePropName: 'fileList',
                            getValueFromEvent: this.normFile,
                            rules: [{ validator: this.checkUploadPic('insideButtonPic') }],
                            initialValue: insideButtonPic ? this.getDefaultFileListByKey('insideButtonPic') : '',
                          })(<Upload
                            {...uploadConfig}
                            onChange={this.onUploadChange('insideButtonPic')}
                            beforeUpload={this.beforeUpload(981, 165, 50)}
                            onRemove={this.onRemove('insideButtonPic')}
                          >
                            <Button>
                              <Icon type="upload" style={f15} /> 上传文件
                            </Button>
                  ...
                  <PreviewWrapper backgroundColor={backgroundColor} className={`${uiClass}`}>
                    ...
                    <div className="radius-section">
                      {
                        supportShare === 2 && oldAcceptRule === 2
                          ? '' : <p className="title">恭喜您领到红包</p>
                      }
                      {
                        (supportShare === 2 && oldAcceptRule === 2)
                          ? '' : (
                ...
                <FormItem {...formItemLayout} label="头图">
                  {
                    getFieldDecorator('outsideHeadPic', {
                      ...
                      rules: [{ validator: this.checkUploadPic('outsideHeadPic') }],
                      initialValue: outsideHeadPic ? this.getDefaultFileListByKey('outsideHeadPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsideHeadPic')}
                      beforeUpload={this.beforeUpload(1125, 1272, 400)}
                      onRemove={this.onRemove('outsideHeadPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="领取button">
                  {
                    getFieldDecorator('outsideButtonPic', {
                      ...
                      getValueFromEvent: this.normFile,
                      // rules: [{ required: true, message: '请上传文件' }],
                      rules: [{ validator: this.checkUploadPic('outsideButtonPic') }],
                      initialValue: outsideButtonPic ? this.getDefaultFileListByKey('outsideButtonPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsideButtonPic')}
                      beforeUpload={this.beforeUpload(885, 165, 50)}
                      onRemove={this.onRemove('outsideButtonPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="领取button(无手机号)">
                  {
                    getFieldDecorator('outsideButtonWithoutPhonePic', {
                      ...
                      getValueFromEvent: this.normFile,
                      // rules: [{ required: true, message: '请上传文件' }],
                      rules: [{ validator: this.checkUploadPic('outsideButtonWithoutPhonePic') }],
                      initialValue: outsideButtonWithoutPhonePic
                        ? this.getDefaultFileListByKey('outsideButtonWithoutPhonePic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsideButtonWithoutPhonePic')}
                      beforeUpload={this.beforeUpload(885, 165, 50)}
                      onRemove={this.onRemove('outsideButtonWithoutPhonePic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
          ...
          {supportShare === 2
            && (
              <div className="rel" style={mb250}>
                ...
                <FormItem {...formItemLayout} label="头图">
                  {
                    getFieldDecorator('outsidePrizeOldHeadPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('outsidePrizeOldHeadPic') }],
                      initialValue: outsidePrizeOldHeadPic ? this.getDefaultFileListByKey('outsidePrizeOldHeadPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeOldHeadPic')}
                      beforeUpload={this.beforeUpload(1125, 480, 400)}
                      onRemove={this.onRemove('outsidePrizeOldHeadPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                {
                  outsidePrizeOldIsShareButton === 2
                  && (
                  <FormItem {...tailFormItemLayout} label="">
                    ...
                    getFieldDecorator('outsidePrizeOldIsShareButtonPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('outsidePrizeOldIsShareButtonPic') }],
                      initialValue: outsidePrizeOldIsShareButtonPic
                        ? this.getDefaultFileListByKey('outsidePrizeOldIsShareButtonPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeOldIsShareButtonPic')}
                      beforeUpload={this.beforeUpload(160, 50, 50)}
                      onRemove={this.onRemove('outsidePrizeOldIsShareButtonPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="引流回站button">
                  {
                    getFieldDecorator('outsidePrizeOldButtonPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('outsidePrizeOldButtonPic') }],
                      initialValue: outsidePrizeOldButtonPic
                        ? this.getDefaultFileListByKey('outsidePrizeOldButtonPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeOldButtonPic')}
                      beforeUpload={outsidePrizeOldIsShareButton === 2 ? this.beforeUpload(160, 50, 50) : this.beforeUpload(981, 165, 50)}
                      onRemove={this.onRemove('outsidePrizeOldButtonPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <PreviewWrapper backgroundColor={backgroundColor} style={{ overflow: 'hidden' }} className={`${uiClass}`}>
                  <div className="scroll-wrapper">
                    <p className="bgwh tc mb0">{outsidePrizeOldTitle}</p>
                    <img
                      src={outsidePrizeOldHeadPic}
                      style={w375}
                      alt=""
                    />
                    <div className="radius-section">
                    ...
                    {
                      externalOldRandomType === 1
                      && (
                        <RuleSection text="看看朋友手气如何">
                  ...
                  <div className="bottom-wrapper">
                    <img
                      style={{ width: outsidePrizeOldIsShareButton === 2 ? '50%' : '100%' }}
                      src={outsidePrizeOldButtonPic}
                      alt=""
                    />
                    {outsidePrizeOldIsShareButton === 2 && (
                    <img
                      style={{ width: '50%' }}
                      src={outsidePrizeOldIsShareButtonPic}
                      alt=""
                    />
                    )}
                  </div>
                ...
                <FormItem {...formItemLayout} label="头图">
                  {
                    getFieldDecorator('outsidePrizeEndHeadPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('outsidePrizeEndHeadPic') }],
                      initialValue: outsidePrizeEndHeadPic ? this.getDefaultFileListByKey('outsidePrizeEndHeadPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeEndHeadPic')}
                      beforeUpload={this.beforeUpload(1125, 1272, 400)}
                      onRemove={this.onRemove('outsidePrizeEndHeadPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="引流回站button">
                  {
                    getFieldDecorator('outsidePrizeEndButtonPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('outsidePrizeEndButtonPic') }],
                      initialValue: outsidePrizeEndButtonPic
                        ? this.getDefaultFileListByKey('outsidePrizeEndButtonPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeEndButtonPic')}
                      beforeUpload={this.beforeUpload(981, 165, 50)}
                      onRemove={this.onRemove('outsidePrizeEndButtonPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="头图">
                  {
                    getFieldDecorator('outsidePrizeNewHeadPic', {
                      valuePropName: 'fileList',
                      getValueFromEvent: this.normFile,
                      rules: [{ validator: this.checkUploadPic('outsidePrizeNewHeadPic') }],
                      initialValue: outsidePrizeNewHeadPic ? this.getDefaultFileListByKey('outsidePrizeNewHeadPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeNewHeadPic')}
                      beforeUpload={this.beforeUpload(1125, 1272, 400)}
                      onRemove={this.onRemove('outsidePrizeNewHeadPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="新用户获奖文案">
                  {
                    getFieldDecorator('outsideNewAwardCopyWriter', {
                      rules: [{ required: true, message: '请填写新用户获奖文案' }],
                ...
                <FormItem {...formItemLayout} label="引流回站button">
                  {
                    getFieldDecorator('outsidePrizeNewButtonPic', {
                      ...
                      getValueFromEvent: this.normFile,
                      // rules: [{ required: true, message: '请上传文件' }],
                      rules: [{ validator: this.checkUploadPic('outsidePrizeNewButtonPic') }],
                      initialValue: outsidePrizeNewButtonPic
                        ? this.getDefaultFileListByKey('outsidePrizeNewButtonPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      onChange={this.onUploadChange('outsidePrizeNewButtonPic')}
                      beforeUpload={this.beforeUpload(981, 165, 50)}
                      onRemove={this.onRemove('outsidePrizeNewButtonPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
            ...
            <FormItem {...formItemLayout} label="头图">
              {
                getFieldDecorator('endHeadPic', {
                  valuePropName: 'fileList',
                  getValueFromEvent: this.normFile,
                  rules: [{ validator: this.checkUploadPic('endHeadPic') }],
                  initialValue: endHeadPic ? this.getDefaultFileListByKey('endHeadPic') : '',
                })(<Upload
                  {...uploadConfig}
                  onChange={this.onUploadChange('endHeadPic')}
                  beforeUpload={this.beforeUpload(1125, 1272, 400)}
                  onRemove={this.onRemove('endHeadPic')}
                >
                  <Button>
                    <Icon type="upload" style={f15} /> 上传文件
                  </Button>
            ...
            <FormItem {...formItemLayout} label="活动结束文案">
              {
                getFieldDecorator('endDesc', {
                  rules: [{ required: true, message: '请填写活动结束文案' }],
            ...
            <FormItem {...formItemLayout} label="引流回站button">
              {
                getFieldDecorator('endButtonPic', {
                  valuePropName: 'fileList',
                  getValueFromEvent: this.normFile,
                  rules: [{ validator: this.checkUploadPic('endButtonPic') }],
                  initialValue: endButtonPic ? this.getDefaultFileListByKey('endButtonPic') : '',
                })(<Upload
                  {...uploadConfig}
                  onChange={this.onUploadChange('endButtonPic')}
                  beforeUpload={this.beforeUpload(981, 165, 50)}
                  onRemove={this.onRemove('endButtonPic')}
                >
                  <Button>
                    <Icon type="upload" style={f15} /> 上传文件
                  </Button>
                ...
                <FormItem {...formItemLayout} label="H5分享图标">
                  {
                    getFieldDecorator('insideIconPic', {
                      ...
                      getValueFromEvent: this.normFile,
                      // rules: [{ required: true, message: '请上传文件' }],
                      rules: [{ validator: this.checkUploadPic('insideIconPic') }],
                      initialValue: insideIconPic ? this.getDefaultFileListByKey('insideIconPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      className="upload-list-inline"
                      listType="picture"
                      onChange={this.onUploadChange('insideIconPic')}
                      beforeUpload={this.beforeUpload(150, 150, 10)}
                      onRemove={this.onRemove('insideIconPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
                ...
                <FormItem {...formItemLayout} label="小程序分享图标">
                  {
                    getFieldDecorator('appletIconPic', {
                      ...
                      getValueFromEvent: this.normFile,
                      // rules: [{ required: true, message: '请上传文件' }],
                      rules: [{ validator: this.checkUploadPic('appletIconPic') }],
                      initialValue: appletIconPic ? this.getDefaultFileListByKey('appletIconPic') : '',
                    })(<Upload
                      {...uploadConfig}
                      className="upload-list-inline"
                      listType="picture"
                      onChange={this.onUploadChange('appletIconPic')}
                      beforeUpload={this.beforeUpload(630, 504, 200)}
                      onRemove={this.onRemove('appletIconPic')}
                    >
                      <Button>
                        <Icon type="upload" style={f15} /> 上传文件
                      </Button>
          ...
          <>
            ...
            <FormItem {...formItemLayout} label="活动进度通知">
              ...
              getFieldDecorator('activityProgressNotification', {
                rules: [{ required: true, message: '请选择是否开启微信小程序活动通知' }],
            ...
            <FormItem {...formItemLayout} label="优惠到帐通知">
              ...
              getFieldDecorator('noticeActivityReward', {
                rules: [{ required: true, message: '请选择是否开启优惠到帐通知' }],
        ...
        <Row style={bottomStyle}>
          <Col offset={7}>
            ...
            {
              this.props.type === 'viewActivityDetail'
                ? ''
                : (
                  <Button
                    type="primary"
...
const WrappedActivityStepTwo = Form.create({
  onFieldsChange(props, fields) {
    ...
    const ignoreKeys = ['insideHeadPic', 'insideButtonPic', 'insideIconPic', 'appletIconPic', 'outsideButtonPic',
      'outsideHeadPic', 'outsidePrizeOldHeadPic', 'outsidePrizeOldButtonPic', 'outsidePrizeOldIsShareButtonPic',
      'outsidePrizeNewHeadPic', 'outsidePrizeNewButtonPic', 'endHeadPic', 'endButtonPic', 'carIconPic',
      'appWaitIconPic', 'appWaitIconPicOfMessage', 'miniProgramWaitIconPic', 'outsidePrizeEndButtonPic', 'outsidePrizeEndHeadPic', 'outsideButtonWithoutPhonePic',
      'evaluationDialogPic', 'travelingRedBagIconPic', 'evaluationBannerPic',
    ];
